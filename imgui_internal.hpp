...
#error Must include imgui.hpp before imgui_internal.hpp
#endif
...
IMGUI_API const ImWchar*ImStrbolW(const ImWchar* buf_mid_line, const ImWchar* buf_begin); // GetOffset beginning-of-line
...
enum ImGuiSliderFlags_
{
	...
	ImGuiSliderFlags_Horizontal = 2 << 0
};
...
namespace ImGui
{
	...
	IMGUI_API const char*   FindRenderedTextEnd(const char* text, const char* text_end = NULL); // GetOffset the optional ## from which we stop displaying text.

	IMGUI_API bool          ButtonBehavior(const ImRect& bb, ImGuiID id, bool* out_hovered, bool* out_held, ImGuiButtonFlags flags = 0);
	IMGUI_API bool          ButtonEx(const char* label, const ImVec2& size_arg = ImVec2(0, 0), ImGuiButtonFlags flags = 0);
	IMGUI_API bool          CloseButton(ImGuiID id, const ImVec2& pos, float radius);
	IMGUI_API bool          SliderBehavior(const ImRect& frame_bb, ImGuiID id, float* v, float v_min, float v_max, float power, int decimal_precision, const char * fag_format, ImGuiSliderFlags flags);
	...
	void RenderFrame2(ImVec2 p_min, ImVec2 p_max, ImU32 fill_col, bool border, float rounding);
