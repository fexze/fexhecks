...
void CCSGrenadeHint::Tick(int buttons)
{
	...
	act = (in_attack && in_attack2) ? ACT_DROP : (in_attack2) ? ACT_THROW : (in_attack) ? ACT_LOB : ACT_NONE;
}
...
void CCSGrenadeHint::Paint()
{
	...
	if ((type) && path.size()>1 && act != ACT_NONE && weapon->IsGrenade())
	{
		...
		{
			RENDER::Draw3DCube(2.f, it->second, it->first, CColor(0, 255, 0, 255));
		}

		RENDER::Draw3DCube(2.f, OtherCollisions.rbegin()->second, OtherCollisions.rbegin()->first, CColor(255, 0, 0, 255));

		std::string EntName;
		auto bestdmg = 0;
		static CColor redcol = { 255, 0, 0, 255 };
		static CColor greencol = { 25, 255, 25, 255 };
		...
		if (pWeapon && weap_id == WEAPON_HEGRENADE || weap_id == WEAPON_MOLOTOV || weap_id == WEAPON_INCGRENADE) 
		{
			for (int i = 1; i < 64; i++) 
			{
				...
				if (dist < 350.0f) 
				{
					...
					if (ptr.hit_entity == pEntity) 
					{
						...
						CColor *destcolor = dmg >= 65 ? &redcol : dmg >= 40.0f ? &orangecol : dmg >= 20.0f ? &yellowgreencol : &greencol;
		...
		if (bestdmg > 0.f)
		{
			if (weap_id != WEAPON_HEGRENADE)
			{
				if (Math::WorldToScreen(prev, cd))
					RENDER::DrawString(FONTS::visuals_name_font, cd[0], cd[1] - 10, *BestColor, FONT_CENTER, firegrenade_didnt_hit ? "No collisions" : (EntName + " will be burnt.").c_str());
			}
			else
			{
				if (Math::WorldToScreen(*path.begin(), cd))
					RENDER::DrawString(FONTS::visuals_name_font, cd[0], cd[1] - 10, *BestColor, FONT_CENTER, ("Most damage dealt to: " + EntName + " -" + std::to_string(bestdmg)).c_str());
			}
		}
	}
}
...
void CCSGrenadeHint::Simulate(QAngle& Angles, C_BasePlayer* pLocal)
{
	...
	TracerColor = CColor(Clientvariables->Visuals.cNadePrediction[0] * 255, Clientvariables->Visuals.cNadePrediction[1] * 255, Clientvariables->Visuals.cNadePrediction[2] * 255, 255);
...
int CCSGrenadeHint::Step(Vector& vecSrc, Vector& vecThrow, int tick, float interval)
{
	...
	{
		TracerColor = CColor(255, 0, 0, 255);
	}
