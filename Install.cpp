...
IGameEventManager		*g_GameEvents = nullptr;
...
std::unique_ptr<ShadowVTManager> g_pModelRenderHook = nullptr;
...
SendNetMsg_t o_SendNetMsg = nullptr;
...
unsigned long __stdcall Installer::LoadCheat(void *unused)
{
	FUNCTION_GUARD;
	...
	g_GameEvents = Iface::IfaceMngr::getIface<IGameEventManager>("engine.dll", "GAMEEVENTSMANAGER002", true);
	...
	g_pModelRenderHook = std::make_unique<ShadowVTManager>();

	g_pDMEHook->Setup(g_MdlRender);
	g_pD3DDevHook->Setup(g_pD3DDevice9);
	...
	g_pModelRenderHook->Setup(g_MdlRender);
	...
	g_pModelRenderHook->Hook(21, Handlers::DrawModelExecute_h);
	...
	o_DrawModelExecute = g_pModelRenderHook->GetOriginal<DrawModelExecute_t>(21);
