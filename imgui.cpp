...
const ImWchar* ImStrbolW(const ImWchar* buf_mid_line, const ImWchar* buf_begin) // GetOffset beginning-of-line
{
...
// GetOffset window given position, search front-to-back
...
void ImGui::RenderFrame2(ImVec2 p_min, ImVec2 p_max, ImU32 fill_col, bool border, float rounding)
{
	...
	if (border && (window->Flags & ImGuiWindowFlags_ShowBorders))
	{
		window->DrawList->AddRect(p_min + ImVec2(1, 1), p_max + ImVec2(1, 1), GetColorU32(ImGuiCol_Frameout2), rounding);
		window->DrawList->AddRect(p_min, p_max, GetColorU32(ImGuiCol_Frameout2), rounding);
	}
}

bool ImGui::SliderBehavior(const ImRect& frame_bb, ImGuiID id, float* v, float v_min, float v_max, float power, int decimal_precision, const char* fag_format, ImGuiSliderFlags flags)
{
	ImGuiContext& g = *GImGui;
	ImGuiWindow* window = GetCurrentWindow();
	const ImGuiStyle& style = *g.Style;
	...
	// Draw frame
	RenderFrame2(frame_bb.Min, frame_bb.Max, GetColorU32(ImGuiCol_FrameOut), true, style.FrameRounding);
	...
	window->DrawList->AddRectFilled(frame_bb.Min + ImVec2(2, 2), grab_bb.Max - ImVec2(0, 2), GetColorU32(g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab));

	char value_buf[64];
	const char* value_buf_end = value_buf + ImFormatString(value_buf, IM_ARRAYSIZE(value_buf), fag_format, *v);
	RenderTextClipped(ImVec2(grab_bb.Min.x - 100, grab_bb.Max.y - 8), ImVec2(grab_bb.Max.x + 100, grab_bb.Max.y + 100), value_buf, value_buf_end, NULL, GetColorU32(ImGuiCol_Text), ImVec2(0.5f, 0.f));
	return value_changed;
}
...
bool ImGui::SliderFloat(const char* label, float* v, float v_min, float v_max, const char* display_format, float power)
{
	...
	const ImVec2 label_size = CalcTextSize(label, NULL, true);
	const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w, label_size.y - 7 + style.FramePadding.y*0.5f));
	...
	ItemSize(total_bb, style.FramePadding.y);
	const bool value_changed = SliderBehavior(frame_bb, id, v, v_min, v_max, power, decimal_precision, display_format, ImGuiSliderFlags_Horizontal);
...
bool ImGui::VSliderFloat(const char* label, const ImVec2& size, float* v, float v_min, float v_max, const char* display_format, float power)
{
	...
	bool value_changed = SliderBehavior(frame_bb, id, v, v_min, v_max, power, decimal_precision, display_format, ImGuiSliderFlags_Vertical);
...
bool ImGui::Checkbox(const char* label, bool* v)
{
	...
	const ImRect check_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(label_size.y + style.FramePadding.y - 4, label_size.y + style.FramePadding.y - 4));
	...
	RenderFrame2(check_bb.Min, check_bb.Max, GetColorU32((held && hovered) ? ImGuiCol_FrameOut : hovered ? ImGuiCol_FrameOut : ImGuiCol_FrameOut), true, style.FrameRounding);
...
bool ImGui::RadioButton(const char* label, bool active)
{
	...
	window->DrawList->AddCircleFilled(center, radius, GetColorU32((held && hovered) ? ImGuiCol_RadioButtonActive : hovered ? ImGuiCol_RadioButtonHovered : ImGuiCol_RadioButtonBg), 16);

	if (active) 
	{
		const float check_sz = ImMin(check_bb.GetWidth(), check_bb.GetHeight());
		const float pad = ImMax(1.0f, (float)(int)(check_sz / 6.0f));
		window->DrawList->AddCircleFilled(center, radius - pad, GetColorU32(ImGuiCol_RadioButtonDot), 16);
	}
...
bool ImGui::InputTextEx(const char* label, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiTextEditCallback callback, void* user_data)
{
	...
	if (g.ActiveId == id || is_currently_scrolling) {
		...
		{
			// Count lines + GetOffset lines numbers straddling 'cursor' and 'select_start' position.
			...
			// Iterate all lines to GetOffset our line numbers
...
bool ImGui::ColorButton2(const ImVec4& col, bool small_height, bool outline_border)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
	...
	const ImRect bb(ImVec2(window->DC.CursorPos.x, window->DC.CursorPos.y + 6), window->DC.CursorPos + ImVec2(square_size + style.FramePadding.y * 3, square_size + (small_height ? 0 : style.FramePadding.y * 2) - 1.3));
