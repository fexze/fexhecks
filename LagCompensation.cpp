...
bool CMBacktracking::StartLagCompensation(C_BasePlayer* player)
{
	...
	switch (Clientvariables->Rage.iLagCompType)
	{
	case TYPE_BEST_RECORDS:
	{
		for (auto it : m_LagRecords)
		{
			if (it.m_iPriority >= 1 || (it.m_vecVelocity.Length2D() > 35.f))
...
void CMBacktracking::RageBacktrack(C_BasePlayer* target, CUserCmd* usercmd, Vector& aim_point, bool& hitchanced)
{
	if (StartLagCompensation(target))
	{
		...
		while (FindViableRecord(target, &cur_record))
		{
			...
			if (AimRage::Get().Bodyaim(target))
				aim_point = AimRage::Get().CalculateBestPoint(target, HITBOX_BELLY, Clientvariables->Rage.iMinDmg, true, iter->matrix);
			else
				aim_point = AimRage::Get().CalculateBestPoint(target, Clientvariables->Rage.iHitbox, Clientvariables->Rage.iMinDmg, true, iter->matrix);
			...
			QAngle aimAngle = Math::CalcAngle(g_LocalPlayer->GetEyePos(), aim_point) - (Clientvariables->Rage.bRemoveRecoil ? g_LocalPlayer->m_aimPunchAngle() * 2.f : QAngle(0,0,0));
			hitchanced = AimRage::Get().HitChance(aimAngle, target, Clientvariables->Rage.iHitchance);
...
// Better get and use polak's version rather than our old one, since we are getting more accurate results
